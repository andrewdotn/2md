...
describe('2md', function() {
    describe('parsing', function() {
        it('can parse the first sample', async function() {
            ...
            const expected = new Doc(
                ...
                new P(["Mac hardware and macOS made the jump from 32 bits to 64 bits a long time ago, but Catalina will be the very first version of macOS that is totally unable to run 32-bit software. For (what I hope will be) the last time, let's review the Mac's entire 64-bit timeline from start to finish:"]),
                new L([new B(['June 2003']),
                    ...
                    ' and with Mac OS X 10.3 Panther, it can theoretically address up to 8GB of RAM.']),
                        new L([new B(['April 2005']),
                            'Mac OS X 10.4 Tiger ',
                                new A(['allows for 64-bit processes under the hood'], {href: 'https://arstechnica.com/gadgets/2005/04/macosx-10-4/4/'}),
    ...
    describe('end-to-end', function () {
        it('can process the first sample', async function () {
            const [html, md] = await Promise.all(['html', 'md'].map(ext =>
                fixture(`quote1.${ext}`)));
            // Not implemented yet
            this.skip();
