...
method. This is how the Ir tree gets turned into a markdown string.
...
Pretty straightforward: add a prefix ofj `"  - "` for the first line the
bullet item and `"    "` for subsequent lines, recursively render the
`IrNode`’s contents, then remove the prefix.
...
html can be correctly handled that way, but there is an issue:
...
ends up writing into OutputBlock objects, with a new OutputBlock
automatically created whenever the prefix changes.

Each OutputBlock contains:

  - some already-markdown text content

  - a stack of prefixes to insert at the beginning of each line
...
Then when something inside a bulleted list is ready for output, deciding
whether to start the line with `"  - "` at the start of the bulleted list
or `"    "` for later lines is as simple as asking the prefix object
whether its first line has already been printed or not. Even if the
something that’s ready for output is quite deeply nested inside the
bulleted list.
...
The structure here is usually pretty boring, but is particularly noticeable
on the ‘Nested lists’ example.
...
other people to use: just click a link and get something interactive. For
the ability to give people that, I will definitely use more TypeScript in
the future!

Relating back to the quote from Neal Ford on automation: instead of getting
bored adding markdown formatting by hand, I learned a ton automating the
process, and I’m really, really happy about that. In fact the hard part now
is resisting the urge to simultaneously explore all the new avenues this
has opened up.
