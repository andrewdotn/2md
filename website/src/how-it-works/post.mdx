...
`src/tree-transforms.ts`.
  ...
  - Turn things that started out as `<pre><code>…</code></pre>` into plain
    ...
    which gets rendered into HTML as `<pre><code>…</code></pre>`.

    When going from HTML to markdown, `<code>` should turn
    into <code>&#96;foo&#96;</code>, and 2md has a Code class to represent an
    inline code block. Meanwhile HTML of the form `<pre><code>…</code></pre>`
