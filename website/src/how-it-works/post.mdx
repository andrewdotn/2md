...
note of is that, on order to make the snippet self-contained, the browser
took all the computed styles from all the CSS and inlined it on every
element, making the markup really messy.
...
Well, there’s lots that can be thrown away, like any HTML comments, the
contents of any `class=""` attributes; and we don’t care about `<div>`s or
`<span>`s; … in fact we’re going to throw away everything except for a
small number of tags, and do some normalization such as mapping both
`<i>foo</i>` and `<em>foo</em>` into the same thing because they’ll both
come out the markdown end as `*foo*`.

And where Ir comes into play: it’s a simplified, normalized data structure
to make the HTML input more markdown-like.
